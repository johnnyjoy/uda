...
Accepting a `"driver"` value in JSON does **not** mean that engine is CI integration-gated.
Only **SQLite** and **PostgreSQL** are enforced in GitHub Actions today.

See **`docs/integration/README.md`** for the full matrix (connect, builders, upsert,
cache, CI workflow names). `config/example-config.json` lists several engines for
config-shape illustration — treat non-gated engines as integrator-validated.
...
| Key            | Required | Description                                                                                             |
| -------------- | -------- | ------------------------------------------------------------------------------------------------------- |
| **driver**     | yes      | **Engine** / SQL family: `sqlite`, `pgsql`, `mysql`, `mariadb`, `sqlserver`, `sybase`, `oracle`, … Shorthand aliases (`sqlsrv`, `dblib`, …) normalize to canonical engine + transport. Case-insensitive. **CI integration-gated:** `sqlite`, `pgsql`, `mariadb`, `sqlserver` (`dblib` on Linux CI), `oracle` — see `docs/integration/README.md`. |
