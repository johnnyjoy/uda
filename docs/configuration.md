...
| Key            | Required | Description                                                                                             |
| -------------- | -------- | ------------------------------------------------------------------------------------------------------- |
| **driver**     | yes      | **Engine** / SQL family: `sqlite`, `pgsql`, `mysql`, `mariadb`, `sqlserver`, `sybase`, `oracle`, … Shorthand aliases (`sqlsrv`, `dblib`, …) normalize to canonical engine + transport. Case-insensitive. **CI-certified today:** `sqlite`, `pgsql`, `sqlserver` (`dblib` on Linux CI) — see `docs/certification/README.md`. |
