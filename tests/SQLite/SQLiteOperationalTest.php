...
final class SQLiteOperationalTest extends SQLiteTestCase
{
    ...
    public function testGuardrailBlocksUnsafeDelete(): void
    {
        ...
        $this->guardrailDb(function (Database $db) use ($collector): void {
            try {
    ...
    {
        $this->withMemoryDb(function (Database $db): void {
    ...
    public function testRetryRetriesTransientRead(): void
    {
        ...
        $this->withMemoryDb(function (Database $db) use ($collector): void {
     ...
     * @return TValue
     */
    private function guardrailDb(callable $fn): mixed
    {
        return $this->withMemoryDbConfig([
    ...
    private function registerMetricsAggregator(): MetricsAggregator
    {
    ...
    public function testReplayCaptureAndReplay(): void
    {
        ...
        try {
            $captureDb = $this->bootReplayDatabase(enableReplay: true, directory: $tempDir);
            $rows = $captureDb->rows('SELECT * FROM employees LIMIT 1');
            self::assertNotEmpty($rows);
            Config::clearForTests();
            Database::clearTraceListeners();
            ReplayBootstrapper::reset();

            $files = glob(sprintf('%s/*.ndjson', $tempDir)) ?: [];
            self::assertNotEmpty($files, 'Expected replay NDJSON to be written');

            $ndjsonPath = $files[0];
            $lines = array_values(array_filter(array_map('trim', file($ndjsonPath) ?: [])));
            self::assertNotEmpty($lines, 'Expected NDJSON entries');
            ...
            $replayDb = $this->bootReplayDatabase(enableReplay: false);
            $replayer = new QueryReplayer($replayDb);
            $result = $replayer->runFile($ndjsonPath);

            self::assertNotEmpty($result, 'Replay should produce queries');
        } finally {
    ...
    {
        $dir = sys_get_temp_dir() . '/sqlite-replay-' . uniqid('', true);
    ...
    {
        $config = [
            'defaults' => ['connection' => 'sqlite_replay'],
            'replay' => [
...
private function writeReplayConfig(array $config): string
    {
        $temp = tempnam(sys_get_temp_dir(), 'uda-sqlite-replay-');
