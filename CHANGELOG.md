...
- Fix Oracle CI `testBuilderDeterminism` child-process crash: replace triple-fetch with compile stability + two fetches; `PreserveGlobalState(false)` on `OracleTestCase`.
